...
use crate::day::{Answer, Day, DayInfo, DaysMeta, Example, SolverDatabase};
...
pub enum RunError {
    ...
    #[error("no meta for {0}")]
    NoMeta(Day),
    #[error("wrong answer for example: expected \"{expected}\" but was \"{actual}\"")]
    ExampleWrongAnswer {
        expected: Answer,
        actual: Answer,
    },
...
fn run_from_args(args: RunArgs, meta: DaysMeta) -> Result<RunResult, RunError> {
    ...
    let reader = source.to_readers(&meta, day)
        .map_err(|e| match e {
        ...
        SourceReader::Example(example) => {
            match example {
                ...
                ExampleReader::Multi { file_1, expected_answer_1, file_2, expected_answer_2 } => {
                    let answer = Answer {
                        part_1: expected_answer_1,
                        part_2: Some(expected_answer_2)
                    };
