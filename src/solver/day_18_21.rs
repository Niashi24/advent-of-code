...
impl SeparatedSolver for Day1821 {
    fn part_1(&self, input: Box<dyn BufRead>) -> anyhow::Result<String> {
        let r = input.lines().map(Result::unwrap)
    ...
    fn part_2(&self, input: Box<dyn BufRead>) -> anyhow::Result<String> {
        let numbers = input.lines().map(Result::unwrap)
            .map(|s| Number::from_str(&s))
            .collect_vec();
        
        let part_2 = numbers.iter().cartesian_product(numbers.iter())
...
impl Number {
    fn explode(&mut self, depth: u32, left: Option<&mut u32>, right: Option<&mut u32>)
               -> bool {
        if depth >= 4 {
            ...
            if let Some(l) = left {
                let Number::Num(a) = a.as_ref() else { panic!("a was not num"); };
                *l += *a;
            }
            if let Some(r) = right {
                let Number::Num(b) = b.as_ref() else { panic!("b was not num"); };
                *r += *b;
            }
