...
impl CombinedSolver for Day21 {
    fn solve(&self, input: Box<dyn BufRead>) -> anyhow::Result<(String, String)> {
        ...
        let p_1: u32 = p_1.strip_prefix("Player 1 starting position: ").unwrap().parse().unwrap();
        let p_2 = lines.next().unwrap();
        let p_2: u32 = p_2.strip_prefix("Player 2 starting position: ").unwrap().parse().unwrap();
...
fn run(mut p_1: State, mut p_2: State) -> usize {
    ...
    loop {
        ...
        if p_1.1 >= 1000 { return dice.n * (p_2.1 as usize); }
        p_2 = step(p_2, dice.next_total());
        if p_2.1 >= 1000 { return dice.n * (p_1.1 as usize); }
    }
}
...
fn recurse(p_1: State, p_2: State) -> (usize, usize) {
    ...
    for (r_1, m_1) in ROLLS {
        for (r_2, m_2) in ROLLS {
            ...
            if p_1.1 >= 21 { w_1 += m_1; break; }
            let p_2 = step(p_2, r_2);
            if p_2.1 >= 21 { w_2 += m_2; continue; }
