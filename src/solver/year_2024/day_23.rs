...
pub fn solve(input: Box<dyn BufRead>) -> anyhow::Result<(impl Display, impl Display)> {
    ...
    let mut p_1 = 0;
    for (a, b, c) in graph.nodes()
        .tuple_combinations() {
        if ![a[0],b[0],c[0]].iter().any(|c| *c == 't') {
            continue;
        }
        if !graph.contains_edge(a, b) || !graph.contains_edge(b, c)
    ...
    best.sort();
    let p_2 = best.into_iter().map(|c| format!("{}{}", c[0], c[1])).join(",");
    
    Ok((p_1, p_2))
}

fn find(mut this: Vec<Node>, mut idx: usize, remaining: usize, graph: &Graph, nodes: &IndexSet<Node>) -> (Vec<Node>, bool) {
    if remaining == 0 {
        return (this, true);
    }
