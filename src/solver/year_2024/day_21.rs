...
impl Direction {
    ...
    fn transition_path(a: Self, b: Self) -> SmallVec<[SmallVec<[Direction; 3]>; 3]> {
        ...
        match (a, b) {
            ...
            (D::Up, D::Right) => {
                smallvec![smallvec![D::Down, D::Right], smallvec![D::Right, D::Down]]
            } // multiple
            ...
            (D::Left, D::A) => smallvec![
                smallvec![D::Right, D::Right, D::Up],
                smallvec![D::Right, D::Up, D::Right]
            ],
            ...
            _ => Self::transition_path(b, a)
                .into_iter()
                .map(|s| s.into_iter().rev().map(Direction::reverse).collect())
                .collect(),
        }
    }
...
fn all_transitions(inputs: impl Iterator<Item = IVec2>) -> Vec<Vec<Direction>> {
    ...
    for next in inputs {
        ...
        out = out
            .into_iter()
            .flat_map(|s| {
                transitions.clone().into_iter().map(move |x| {
                    let mut a = s.clone();
                    a.extend(x);
                    a.push(Direction::A);
                    a
...
fn numeric_transition(a: IVec2, b: IVec2) -> SmallVec<[SmallVec<[Direction; 5]>; 2]> {
    match (a.x.cmp(&b.x), a.y.cmp(&b.y)) {
        ...
        (Ordering::Equal, Ordering::Greater) => {
            smallvec![smallvec![Direction::Up; (a.y - b.y) as usize]]
        }
        (Ordering::Equal, Ordering::Less) => {
            smallvec![smallvec![Direction::Down; (b.y - a.y) as usize]]
        }
        (Ordering::Greater, Ordering::Equal) => {
            smallvec![smallvec![Direction::Left; (a.x - b.x) as usize]]
        }
        (Ordering::Less, Ordering::Equal) => {
            smallvec![smallvec![Direction::Right; (b.x - a.x) as usize]]
        }
...
fn solve(seq: &str, depth: usize) -> u64 {
    let all_transitions = all_transitions(seq.chars().map(|c| {
        IVec2::from(match c {
            '7' => [0, 0],
            '8' => [1, 0],
            '9' => [2, 0],
